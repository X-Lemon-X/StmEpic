...
// const std::string stmepic::memory::base_encryption_key = "stmepic";

FRAM::FRAM()
{
  // What is it hardcoding the encryption key?
  const std::string base_encryption_key = "stmepic";
...
Result<std::vector<uint8_t>> FRAM::decode_data(const std::vector<uint8_t> &data)
{
  ...
  uint32_t encres = (decrypted_encres[0] << 24) | (decrypted_encres[1] << 16) | (decrypted_encres[2] << 8) | decrypted_encres[3];
  std::string key = std::to_string(encres) + encryption_key;
