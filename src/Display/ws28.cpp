...
Status WS28Base::update_pixels(const std::vector<Color> &pixels) {
  ...
  if(HAL_TIM_PWM_Start_DMA(&htim, timer_channel, (uint32_t *)pwm_buffor.get(), 24 * settings.pixelCount + reset_cycles_number) != HAL_OK) {
...
Status WS28Base::device_start() {
  // Doesn't require start
  return Status::OK();
}

Status WS28Base::device_stop() {
  // Doesn't reqire stop
  return Status::OK();
}

Status WS28Base::device_reset() {
  // Doesn't have start/stop functionality
  return Status::OK();
}
