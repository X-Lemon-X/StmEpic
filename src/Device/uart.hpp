...
public:
  ...
  /**
   * @brief Read data from the UART device in blocking mode with other tasks beeing able to freelu run in the
   ...
   * Note its beter to use higher timeout them small one otherwise weard things might happen.
   * @return Result<uint8_t *>
   */
  Status read(uint8_t *data, uint16_t size, uint16_t timeout_ms = 300);

  /**
   * @brief Write data to the UART device in blocking mode with other tasks beeing able to freelu run in the
   *
