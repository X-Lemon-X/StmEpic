...
void CAN::task_rx(void *arg) {
  ...
  while(true) {
    ...
    vPortEnterCritical();
    if(can->callbacks.find(msg->frame_id) != can->callbacks.end()) {
      task = &can->callbacks[msg->frame_id];
    } else {
...
Status CAN::write(const CanDataFrame &msg) {
  CanDataFrame *msg_s = (CanDataFrame *)malloc(sizeof(CanDataFrame));
  if(msg_s == nullptr)
    return Status::OutOfMemory("Can't allocate memory for CanDataFrame");
  *msg_s = msg;
  if(xQueueSend(tx_queue_handle, &msg_s, pdMS_TO_TICKS(10)) != pdTRUE) {
    free(msg_s);
...
void CAN::rx_callback(CAN_HandleTypeDef *hcan) {
  if(hcan->Instance != _hcan->Instance || !is_initiated)
    return;
  CanDataFrame *msg = (CanDataFrame *)malloc(sizeof(CanDataFrame));
  if(msg == nullptr)
    return;
  ...
  if(xQueueSendFromISR(rx_queue_handle, &msg, &hptw) != pdTRUE) {
    free(msg);
  }
